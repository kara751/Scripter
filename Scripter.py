...
def openf(event=""):
    ...
    if file =="":
        ...
        root.title(os.path.basename(file)+ "- Scripter")
        area.delete(1.0,END)
        f=open(file,"r")
...
def notepad():
    tmsg.showinfo("Scripter","This is a simple Scripter developed by Karan")
def zoomin():
     for i in range(1,50):
...
global area
area = Text(root,yscrollcommand = scrollbar.set,xscrollcommand=hscrollbar.set,bg="#380474",fg="white",font="TimesNewRoman 16",undo=True,wrap=WORD,selectbackground="black")
...
help.add_command(label="About Scripter",command=notepad)
